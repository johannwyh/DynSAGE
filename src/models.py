...
class GraphSage(nn.Module):
	...
	def __init__(self, args, num_layers, input_size, out_size, raw_features, adj_lists, device, gcn=False, agg_func='MEAN', weighted_aggr=False, n_nodes=0, n_labels=0):
		...
		if self.args.DynamicSAGE:
			self.n_nodes = n_nodes
			self.n_labels = n_labels
			self.all_logists = torch.zeros((self.n_nodes, self.n_labels)).to(device)
	...
	def load_from_object(self, sage_tilde):
		for index in range(1, self.num_layers+1):
			layer = getattr(self, 'sage_layer'+str(index))
			layer_tilde = getattr(sage_tilde, 'sage_layer'+str(index))
			layer.load_state_dict(layer_tilde.state_dict())
